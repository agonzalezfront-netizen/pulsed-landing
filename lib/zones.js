...
export function zSphereLeft(i, n) {
  ...
  return {
    x: 0.26 + Math.cos(angle) * 0.30 * shell,
    y: 0.52 + Math.sin(angle) * 0.30 * shell * 1.05
  };
}
...
export function zGalaxy(i, n) {
  ...
  if (i < n * 0.08) {
    ...
    return { x: cx + Math.cos(a) * r, y: cy + Math.sin(a) * r * ell };
  }
  ...
  // ── 3. Diffuse disk: 18-32% — fills gaps between arms ──
  if (i < n * 0.32) {
    var a = rand(0, Math.PI * 2);
  ...
  // ── 4. Two spiral arms: 68% — thick visible sweeping bands ──
  var t = (i - n * 0.32) / (n * 0.68);
  var arm = i % 2;
  var armStart = arm * Math.PI;

  var angle = armStart + Math.pow(t, 0.65) * Math.PI * 2.2;
  var r = R * 0.10 + t * R * 0.88;

  var thickness = 0.035 + t * 0.09;
  var perp = angle + Math.PI * 0.5;
  var off = rand(-1, 1) * thickness;

  return {
    x: cx + Math.cos(angle) * r + Math.cos(perp) * off,
    y: cy + (Math.sin(angle) * r + Math.sin(perp) * off) * ell
  };
}
