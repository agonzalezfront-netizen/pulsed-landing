...
export class ParticleField {
  constructor(options = {}) {
    ...
    this.pts    = [];
    this.morphFrom = null; this.morphTo = null; this.morphT = 1;
  ...
  morphTo({ zone, color, bg } = {}) {
    ...
    this.morphTo   = Array.from({ length: this.count }, (_, i) => {
      const pos = zone(i, this.count);
      return {
        bx: Math.max(0, Math.min(1, pos.x)),
        by: Math.max(0, Math.min(1, pos.y)),
      };
    });
  ...
  _loop(ts) {
    ...
    // Morph progress
    if (this.morphT < 1 && this.morphFrom && this.morphTo) {
      this.morphT = Math.min(1, this.morphT + dt / this.MORPH_DUR);
      const e = this._easeInOut(this.morphT);
      for (let i = 0; i < N; i++) {
        pts[i].bx = this.morphFrom[i].bx + (this.morphTo[i].bx - this.morphFrom[i].bx) * e;
        pts[i].by = this.morphFrom[i].by + (this.morphTo[i].by - this.morphFrom[i].by) * e;
      }
    }
    ...
    for (let i = 0; i < N; i++) {
      ...
      ctx.globalAlpha  = a;
      ctx.shadowColor  = col;
      ctx.shadowBlur   = p.r > 1.3 ? r*2.5 : r*1.2;
      ctx.fillStyle    = col;
