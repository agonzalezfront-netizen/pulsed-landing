...
export class ParticleField {
  constructor(options = {}) {
    ...
    // Kill rendering + free GPU when tab is hidden
    document.addEventListener('visibilitychange', () => {
      this._paused = document.hidden;
      if (this._paused) {
        this.ctx.clearRect(0, 0, this.VW, this.VH);
        this.cv.width = 1;    // shrink canvas → frees GPU buffer entirely
        this.cv.height = 1;
      } else {
        this._resize();        // restore full canvas size
        this._prev = null;
        requestAnimationFrame(this._loop);
      }
    });
  ...
  _loop(ts) {
    ...
    for (let i = 0; i < N; i++) {
      ...
      if (p.r > 1.3) {
        // Sparkle: halo + shadow (only ~8% of particles)
        ctx.save();
        ...
        // Normal: simple dot, no shadow (92% of particles — fast)
        ctx.globalAlpha = a;
        ctx.fillStyle   = col;
        ctx.beginPath(); ctx.arc(x,y,r,0,Math.PI*2); ctx.fill();
      }
